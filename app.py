...
def _ws_recv(ws, timeout=WS_RECV_POLL):
    """
    curl_cffi WS: recv() bloklayıcı, tuple döndürür (data, flags).
    Kısa timeout'lu polling için recv_timeout kullanıyoruz; yoksa fallback.
    Timeout durumunda None döner.
    """
    try:
        # curl_cffi >=0.7 recv(timeout=...) destekler
...
def stream_message(
    ...
    for retry_cycle in range(2):
        ...
        connect_err = None
        try:
            # curl_cffi native WS — Chrome 120 fingerprint
